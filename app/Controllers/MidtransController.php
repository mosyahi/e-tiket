...
class MidtransController extends BaseController
{
    ...
    public function index_penjual($orderId)
    {
        MidtransConfig::$serverKey = config('Midtrans')->serverKey;
        MidtransConfig::$clientKey = config('Midtrans')->clientKey;
        MidtransConfig::$isProduction = false;
    ...
    public function update_penjual($orderId)
    {
        $status = $this->request->getPost('status');
        $paymentMethod = $this->request->getPost('payment_method');
        ...
        if (session('role') == 2) {
            return redirect()->to('penjual/payment/billing/SdzC64i0Nt8mRsEUK6eK1q4npbrh1pDI128sEi35WPCrz97eVB')->with('success', 'Pembayaran berhasil!');
        } else {
            return redirect()->to('pembeli/payment/billing/SdzC64i0Nt8mRsEUK6eK1q4npbrh1pDI128sEi35WPCrz97eVB')->with('success', 'Pembayaran berhasil!');
        }
    }
